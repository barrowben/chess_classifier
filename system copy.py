...
def process_training_data(fvectors_train: np.ndarray, labels_train: np.ndarray) -> dict:
    ...
    # plt.show()
    chess_train_data = fvectors_train[1::2,:]
    chess_train_labels = labels_train[1::2]
    chess_test_data = fvectors_train[0::2,:]
    chess_test_labels = labels_train[0::2]

    # Divergence between king and pawn
    kdata = chess_train_data[chess_train_labels == "k", :]
    pdata = chess_train_data[chess_train_labels == "p", :]
    d12 = divergence(kdata, pdata)
    im = plt.matshow(np.reshape(d12, (50, 50), order="C"), cmap=cm.Greys)
    plt.colorbar(im)
    plt.show()
    ...
    # Train the model
    predicted_labels = nearest_neighbour(chess_test_data, chess_train_data, chess_test_labels, chess_train_labels, features)

    
    model = {}
    model["labels_train"] = predicted_labels.tolist()
    fvectors_train_reduced = reduce_dimensions(fvectors_train, model)
    model["fvectors_train"] = fvectors_train_reduced.tolist()
    return model

def nearest_neighbour(chess_test_data, chess_train_data,  chess_test_labels, chess_train_labels, features=None):
    if features is None:
    ...
    score = (100.0 * sum(chess_test_labels[:] == predicted_labels)) / predicted_labels.shape[0]
    print(score)
    print("ACTUAL")
    print(chess_test_labels[0:100])
